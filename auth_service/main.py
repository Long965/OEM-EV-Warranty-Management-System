...
from shared.db import engine, Base  # ✅ Thêm dòng này
...
def startup_event():
    ...
    try:
        if session.query(models.Role).count() == 0:
            ...
            default_roles = [
                models.Role(role_name="Admin", description="Administrator"),
                models.Role(role_name="SC_Staff", description="Service Center Staff"),
                models.Role(role_name="SC_Technician", description="Service Center Technician"),
                models.Role(role_name="EVM_Staff", description="EV Manufacturer Staff")
            ]
