...
# định nghĩa các Class (Models)

from sqlalchemy import Column, Integer, String, Date, Text, ForeignKey
from sqlalchemy.orm import declarative_base, relationship

# 1. Base Class để tất cả các model kế thừa
Base = declarative_base()

# 2. Model Khách hàng (Customer)
class Customer(Base):
...
# 4. Model Phụ tùng (VehicleComponent)
...
# 5. Model Người dùng (User/Technician)
class User(Base):
    __tablename__ = 'users'
    
    users_id = Column(Integer, primary_key=True)
    name = Column(String(100))
    email = Column(String(100), unique=True) # Đã sửa lỗi chính tả "emaill"
...
# 6. Model Lịch sử Dịch vụ (ServiceHistory)
