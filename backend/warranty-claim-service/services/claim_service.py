...
def create_claim(db: Session, data: WarrantyClaimCreate, user_id: str):
    claim = WarrantyClaim(
        ...
        created_by=user_id,
        warranty_cost=getattr(data, "warranty_cost", None)
    ...
    try:
        ...
        db.commit()
        db.refresh(claim)
        log_history(db, claim, "Tạo mới phiếu (chờ duyệt)", user_id, "user")
        return claim
    except Exception:
        db.rollback()
...
def update_status(db: Session, claim_id: int, status: ClaimStatus, approver_id: str = None):
    ...
    try:
        ...
        action_text = {
            ClaimStatus.approved: "Duyệt phiếu",
            ClaimStatus.rejected: "Từ chối phiếu",
            ClaimStatus.pending: "Chờ duyệt"
        }.get(status, "Cập nhật trạng thái")
